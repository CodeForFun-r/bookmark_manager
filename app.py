...
def index():
    ...
    bookmark_list = []
    with open('./bookmark_files/homepage2.txt', 'r') as f2:
        for lines in f2.readlines():
            bookmark_list.append(tuple(lines.strip('\n').strip('{').strip('}').split('^')))

    bookmarks2 = dict(bookmark_list)
    return render_template('index.html', bookmarks=bookmarks, bookmarks2=bookmarks2)
